...
import { FunnelTemplate } from '@/data/funnelTemplates';
...
export function CreateFunnelModal({ isOpen, onClose, selectedTemplate }: CreateFunnelModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      const defaultName = selectedTemplate?.name || 'New Campaign';
      setName(defaultName);
      setSlug(
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedTemplate) {
      formData.append('theme', selectedTemplate.theme || 'ethereal');
      formData.append('welcome_title', selectedTemplate.hero?.headline || '');
      formData.append('welcome_description', selectedTemplate.hero?.subheadline || '');
      
      const formattedQuestions = (selectedTemplate.questions || []).map(q => ({
        id: q.id,
        question: q.question,
        type: q.type || 'choice',
        options: q.options || []
      ...
      const storyData = {
        resultTitle: selectedTemplate.resultDefault?.title || 'Your Match',
        resultDesc: selectedTemplate.resultDefault?.description || '',
        ctaLabel: selectedTemplate.resultDefault?.ctaLabel || 'Action',
        ctaType: selectedTemplate.resultDefault?.ctaType || 'whatsapp',
        resultRules: selectedTemplate.resultRules || [],
        goal: selectedTemplate.goal || 'leads',
        selectedIndustry: selectedTemplate.industry || ''
      };
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
          ...
          <div className="p-6 pt-0 bg-white">
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-8 py-4 bg-black text-white text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-indigo-600 active:scale-[0.98] transition-all shadow-xl disabled:opacity-50"
            >
              {isSubmitting ? (
                <><Loader2 size={16} className="animate-spin" /> Deploying Engine...</>
              ) : (
                <>Deploy {selectedTemplate ? selectedTemplate.name : 'Funnel'} <ArrowRight size={16} /></>
              )}
            </button>
          </div>
        </form>
